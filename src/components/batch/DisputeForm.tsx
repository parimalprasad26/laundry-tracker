...
function Counter({ value, max, onChange }: {
  value: number
  max: number
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={cn(
        ...
        value > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-muted-foreground'
      )}>
        {value}
      </span>
      <button
...
export function DisputeForm({ item, open, onClose, onSaved }: Props) {
  ...
  function handleSave() {
    if (damaged === 0) {
      toast.error('Specify how many items were damaged')
      return
    }
    ...
    <Sheet open={open} onOpenChange={v => { if (!v) onClose() }}>
      <SheetContent side="bottom" className="rounded-t-2xl pb-8">
        ...
        <div className="mt-5 flex gap-2">
          ...
          <Button className="flex-1" onClick={handleSave} disabled={isPending || damaged === 0}>
